...
for (const col of Object.values(chronicle.data)) {
  ...
  for (const item of col.items) {
    Deno.test(`${col.name}/${item.id}`, () => {
      ...
      ids.push(item.id)
      // check series
      if (item.index.series && !chronicle.data.series.items.find(s => s.id === item.index.series)) {
        throw `Series not found: ${item.index.series}`
      }
      // check family
      if (item.index.family && !chronicle.data.families.items.find(s => s.id === item.index.family)) {
